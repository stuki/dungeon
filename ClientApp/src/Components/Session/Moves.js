...
import MovesData from "../../Common/moves.json";
import "./Moves.css";
...
const specialMoves = Object.values(MovesData["Special Moves"]).map(
  (m, index) => {
    let [top, list, bottom] = m;
    if (list) {
      list = list.map(item => <ListGroupItem>{item}</ListGroupItem>);
...
function MovesPanel(props) {
  const { heading, top, list, bottom } = props;
  return (
    <Panel>
      <Panel.Heading>{heading}</Panel.Heading>
      <Panel.Body>{top}</Panel.Body>
      {list && <ListGroup>{list}</ListGroup>}
      {bottom && <Panel.Body>{bottom}</Panel.Body>}
    </Panel>
  );
}
  ...
  <div className="moves container">
    <Tabs defaultActiveKey={1}>
      <Tab eventKey={1} title="Basic Moves">
        <Masonry options={masonryOptions}>{basicMoves}</Masonry>
      </Tab>
      <Tab eventKey={2} title="Special Moves">
        <Masonry options={masonryOptions}>{specialMoves}</Masonry>
      </Tab>
    </Tabs>
  </div>
);
